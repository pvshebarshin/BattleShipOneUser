            ...
            if(row == getBowRow()){
                ...
                if(d >= 0 && d < getLength()){
                    ...
                    if(isSunk()){
                        ocean.deadships++;
                        for(int i = 0; i < getLength(); i++) {
                            Button clickedButton = (Button) Main.globalRoot.lookup("#btn"+getBowRow()+(getBowColumn() + i));
                            clickedButton.setStyle("-fx-background-color:black");
                        }
                        for(int i = -1; i < getLength() + 1; i++){
                            try{
                                Button clickedButton = (Button) Main.globalRoot.lookup("#btn"+(getBowRow()+1)+(getBowColumn() + i));
                        ...
                        try {
                            Button clickedButton = (Button) Main.globalRoot.lookup("#btn"+getBowRow()+(getBowColumn()-1));
                            clickedButton.setStyle("-fx-background-color:yellow");
                        }catch (Exception ex){}
                        try {
                            Button clickedButton = (Button) Main.globalRoot.lookup("#btn"+getBowRow()+(getBowColumn() + getLength()));
                            clickedButton.setStyle("-fx-background-color:yellow");
                        }catch (Exception ex){}
            ...
            if(column == getBowColumn()){
                ...
                if(d >= 0 && d < getLength()){
                    ...
                    if(isSunk()){
                        ocean.deadships++;
                        for(int i = 0; i < getLength(); i++) {
                            Button clickedButton = (Button) Main.globalRoot.lookup("#btn"+(getBowRow()+i)+getBowColumn());
                            clickedButton.setStyle("-fx-background-color:black");
                        }
                        for(int i = -1; i < getLength() + 1; i++){
                            try{
                                Button clickedButton = (Button) Main.globalRoot.lookup("#btn"+(getBowRow()+ i)+(getBowColumn()+1 ));
                                clickedButton.setStyle("-fx-background-color:yellow");
                            }catch (Exception ex){ }
                            try{
                                Button clickedButton = (Button) Main.globalRoot.lookup("#btn"+(getBowRow() + i)+(getBowColumn()-1));
                        ...
                        try {
                            Button clickedButton = (Button) Main.globalRoot.lookup("#btn"+(getBowRow()-1)+getBowColumn());
                            clickedButton.setStyle("-fx-background-color:yellow");
                        }catch (Exception ex){}
                        try {
                            Button clickedButton = (Button) Main.globalRoot.lookup("#btn"+(getBowRow()+ getLength())+getBowColumn());
                            clickedButton.setStyle("-fx-background-color:yellow");
                        }catch (Exception ex){}
//                        System.out.printf("You destroyed %s\n", getShipType());
                    }
